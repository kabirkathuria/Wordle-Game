...
;   data types; this is used both for LetterStatusPair a
...
(define (read-dictionary str)
  (if (file-exists? str)
      (map string-upcase (read-words str))
      '()))
...
(define (score input ans)
  (mymap2 (explode input) (mymap2 (explode input) (explode ans) string=?)
          (λ (x y)
            (make-pair x (if y
                             LS-RIGHT
                             (if (string-in-list? x (explode ans))
                                 LS-MISPLACED
                                 LS-WRONG))))))
...
(define (partial-guess->full-guess s len)
  (implode (append (explode s) (build-list (- len (length (explode s))) (λ (x) " ")))))
...
(define (key-wordle ws key los num)
  (cond
    [(key=? key "\b")
     (key-wordle-backspace ws)]
    [(key=? key "\r")
     (key-wordle-return ws los num)]
    [(value-in-list? key (explode "abcdefghijklmnopqrstuvwxyz") string-ci=?)
     (key-wordle-letter ws key los num)]
    [else ws]))
...
; processes a return press given a list of valid words and the required word length
(define (key-wordle-return ws los num)
  (if (or (> (string-length (wordle-current ws)) num)
          (not (string-in-list? (wordle-current ws) los)))
      ws
      (make-wordle (append (wordle-past ws) (list (wordle-current ws))) "")))

; key-wordle-letter : WS KeyEvent [List-of String] Nat -> WS
; processes a keyboard event given a list of valid words and the required word length
(define (key-wordle-letter ws key los num)
  (if (< (string-length (wordle-current ws)) num)
      (make-wordle (wordle-past ws) (string-append (wordle-current ws) (string-upcase key)))
      ws))
...
(define (end-wordle? ws str num)
  (or (>= (length (wordle-past ws)) num)
      (string-in-list? str (wordle-past ws))))
...
(check-expect (draw-end-wordle WS-WORD3 "BAD" 5)
              (overlay/align "left" "top"
                             (text "You Won! :DD\nit took you 3 guesses." 20 "black")
                             (square 400 "solid" BG-COLOR)))

(check-expect (draw-end-wordle WS-WORD3 "NAP" 5)
              (overlay/align "left" "top"
                             (text "You Lost! >:(" 20 "black")
                             (square 400 "solid" BG-COLOR)))

(define (draw-end-wordle ws str num)
  (overlay/align "left" "top"
                 (text (if (and (string-in-list? str (wordle-past ws))
                                (<= (length (wordle-past ws)) num))
                           (string-append "You Won! :DD\nit took you "
...
(check-expect (draw-wordle WS-WORD3 "BAD" 4)
              (overlay
               (stack/v
                (append
                 (map
                  ...
                  (list
                   (list
                    (make-pair (make-pair "C" "wrong") lsp->image)
                    (make-pair (make-pair "A" "right") lsp->image)
                    (make-pair (make-pair "T" "wrong") lsp->image))
                   (list
                    (make-pair (make-pair "D" "misplaced") lsp->image)
                    (make-pair (make-pair "A" "right") lsp->image)
                    (make-pair (make-pair "B" "misplaced") lsp->image))
                   (list
                    (make-pair (make-pair "B" "right") lsp->image)
                    (make-pair (make-pair "A" "right") lsp->image)
                    (make-pair (make-pair "D" "right") lsp->image))))
                 (list (partial-guess->image (wordle-current WS-WORD3) (string-length "BAD")))
                 (build-list
                  (- 4 (+ 1 (length (wordle-past WS-WORD3))))
                  (λ (x)
                    (partial-guess->image "" (string-length "BAD"))))))
               (rectangle (* (string-length "BAD") 72) (* 4 72) "solid" BG-COLOR)))

(check-expect (draw-wordle WS-START "CARTS" 6)
              (overlay
               (stack/v
                (append
                 '()
                 (list (partial-guess->image (wordle-current WS-START) (string-length "CARTS")))
                 (build-list
                  (- 6 (+ 1 (length (wordle-past WS-START))))
                  (λ (x)
                    (partial-guess->image "" (string-length "CARTS"))))))
               (rectangle (* (string-length "CARTS") 72) (* 6 72) "solid" BG-COLOR)))
  ...
  (overlay
   (stack/v
    (append
     ...
     (build-list
      (- num (+ 1 (length (wordle-past ws))))
      (λ (x)
        (partial-guess->image "" (string-length str))))))
   (rectangle (* (string-length str) 72) (* num 72) "solid" BG-COLOR)))
